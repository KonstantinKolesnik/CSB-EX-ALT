...
inline String int64String(uint64_t value, uint8_t base, bool prefix = false, bool sign = false) {
...
inline String int64String(int64_t value, uint8_t base, bool prefix = false) {
  // Signed numbers only make sense for decimal numbers
...
inline String getId() {
  return int64String(ESP.getEfuseMac(), HEX);
}
